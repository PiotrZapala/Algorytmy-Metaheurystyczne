...
def test():
    ...
    for it in range(len(size_of_population)):
        file = open('tests/'+file_name+'-size'+str(size_of_population[it]), 'w')
        file.write('1.(size of population)' + ',' + '2.(probability of mutation)' + ',' + '3.(number of iterations without update)' + ',' + '4.(number of migrations)'  +  ',' + '5.(time)' + ',' + '6.(best solution)' + ',' + '7.(prd)' + '\n')
        ...
        for jt in range(len(probability_of_mutation)):
            ...
            for ka in range(len(number_of_iterations_without_update)):
                ...
                for pe in range(len(number_of_mig)):
                    number_of_migrations = number_of_mig[pe]  
                    best_individual1 = []
                    best_individual2 = []
                    minn = []
                    ...
                    file.write(str(size_of_population[it]) + ',' + str(probability_of_mutation[jt]) + ',' + str(number_of_iterations_without_update[ka]) + ',' + str(number_of_mig[pe]) + ',' + str(my_time) + ',' + str(beesstt) +  ',' + str(prd) + '\n')
